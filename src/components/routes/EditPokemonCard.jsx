...
export default function EditPokemonCard() {
    ...
    useEffect(() => {
        const getPokemonCard = async () => {
            try{
                ...
                setForm(response.data.pokemonCard)
            }catch(err){
                console.warn(err)
                if (err.response) {
    ...
    const handleSubmit = async e => {
        try{
            e.preventDefault()
            const response = await axios.put(`${process.env.REACT_APP_SERVER_URL}/pokemoncards/${id}`, form)
            navigate(`/pokemoncards/${id}`)
        }catch(err){
            console.warn(err)
                if(err.response) {
                    setErrorMessage(err.response.data.message)
